...
export default function PipelineAcquisizione() {
	...
	const [index, setIndex] = useState(1); // The final index set after drag ends
	const containerRefs = useRef<any>([]);

	// Local variable to track index while dragging
	let tmpIndex = index;
	...
	const handleDragEnd = () => {
		setIsDragging(false);
		setIndex(tmpIndex); // Update the main index when dragging ends
	};
	...
	const handleDrag = (event: any, info: any) => {
		containerRefs.current.forEach((ref: any, idx: any) => {
			...
			if (
				...
				tmpIndex = idx; // Only update the local tmpIndex during drag
			}
		});
	};
			...
			<div className="p-4 flex justify-between items-center gap-2">
				<div className="flex gap-2 items-center">
					...
					<motion.button
						...
						whileHover={{ scale: 1.1 }}
						whileTap={{ scale: 0.9 }}
						className="rounded-full p-2 text-white bg-emerald-500"
					>
						<Plus />
					</motion.button>
				</div>
					...
					<div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-4 gap-2 pl-4 pr-4">
						{[12, 13, 14, 15].map(id => (
							...
							>
										...
										{filteredDeals
											...
											.map((d: any, index: any) => (
												<motion.div
													...
													whileHover={{ scale: 1.1 }}
													whileTap={{ scale: 0.9 }}
													className="p-3 bg-emerald-500 hover:bg-emerald-600 rounded-xl text-white mb-2"
								...
								{dealsLoading && (
									<div className="p-3 bg-emerald-slate-200 rounded-xl text-white mb-2" />
								)}
							</div>
						))}
					</div>
