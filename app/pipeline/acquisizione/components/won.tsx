...
export function Won({agency, selectDeal, search}: any) {
	...
	async function getWon() {
		try {
			...
			setWonDeals(data.data);
			setWonCount(data.count);
		} catch (error) {
			console.error('Failed to fetch won deals:', error);
		} finally {
			setWonDealsLoading(false);
		}
	}
							...
							.map((e: any, index: number) => (
								...
								>
									<div className="flex gap-4">
										<p className="flex-1">
											{e.title ? e.title.substring(0, 30) + '...' : '-'}
										</p>
										<p className="flex-1">
										...
										<p className="flex-1">
											{e.value ? formatCurrencyEUR(e.value) : '-'}
										</p>
