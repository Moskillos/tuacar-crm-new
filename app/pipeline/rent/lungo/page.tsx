...
export default function PipelineRentLungo() {
				...
				>
					<div className="relative h-[98%] w-[98%]">
						...
						>
							...
							<Deal session={session} agency={agency} togglePopup={togglePopup} setDeals={setDeals} showDeal={selectedDeal} setSelectedDeal={setSelectedDeal} />
						</motion.div>
					</div>
				</div>
			)}
			...
			<div className="p-4 flex justify-between items-center gap-2">
				<div className="flex gap-2 items-center">
					...
					<motion.button
						...
						whileHover={{ scale: 1.1 }}
						whileTap={{ scale: 0.9 }}
						className="rounded-full p-2 text-slate-800 bg-yellow-300"
					>
						<Plus />
					</motion.button>
				</div>
				<h1 className="font-light text-4xl text-right text-slate-500">
					{deals?.length} affari
				</h1>
			</div>
			...
			<div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-4 gap-2 pl-4 pr-4">
				{[20, 21, 22, 23].map(id => (
					...
					>
								...
								{filteredDeals
									...
									.map((d: any, index: any) => (
										<motion.div
											...
											whileHover={{ scale: 1.1 }}
											whileTap={{ scale: 0.9 }}
											className="p-3 bg-yellow-300 hover:bg-yellow-400 rounded-xl text-slate-800 mb-2"
										>
											<p className="text-xs">
						...
						{dealsLoading && (
							<div className="p-3 bg-yellow-slate-200 rounded-xl text-black mb-2" />
						)}
					</div>
				))}
			</div>
