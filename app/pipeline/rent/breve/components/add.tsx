...
export function Add({ addDealTogglePopup, setDeals, session, rentCars }: any) {
    ...
    //CONTROLLA SE LE DATE SELEZIONATE PER IL NOLEGGIO SONO DISPONIBILI PER L'AUTO SELEZIONATA
    const checkRentDates = () => {
        let disponibile = true;
        rentCars.map((c: any) => {
            if (c.id == deal.carToRentId) {
                console.log(c.id)
                let isoFormatDateFrom = date && date.from ? new Date(date.from) : ""
                let isoFormatDateTo = date && date.to ? new Date(date.to) : ""
                c.rentDays.map((rentDay: any) => {
                    if (rentDay.rentStatus == "rented") {
                        if (new Date(rentDay.start) <= isoFormatDateTo && isoFormatDateFrom <= new Date(rentDay.end)) {
                            disponibile = false
                        }
                    }
                })
            ...
            <div className="w-full mx-auto pt-2 mt-5">
                <div className="flex justify-between items-center gap-2 mb-2">
                    <div className="flex gap-2 items-center w-full">
                        <p className="text-2xl font-semilight mb-2">Nuovo noleggio</p>
                    </div>
                </div>
            </div>
            ...
            <div className="mt-5">
                <div className="rounded-2xl border border-slate-200">
                    <div className="p-4">
                        ...
                        <input type="number" className="font-light rounded-xl border bg-slate-200 border-slate-100 p-2 w-full mb-3" value={deal.value} onChange={(e) => setDeal((prevState) => ({
                            ...prevState,
                        ...
                        <div className="mb-1 grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-4 gap-2">
                            <motion.div onClick={() => setDeal((prevState) => ({
                                ...prevState,
                                stageId: 16
                            }))} className={`rounded-2xl ${deal.stageId == 16 ? "bg-orange-500 text-white" : "bg-slate-200 hover:text-white"} hover:cursor-pointer p-4 hover:bg-orange-500`} whileHover={{ scale: 1.01 }}
                                whileTap={{ scale: 0.9 }}>
                            ...
                            <motion.div onClick={() => setDeal((prevState) => ({
                                ...prevState,
                                stageId: 17
                            }))} className={`rounded-2xl ${deal.stageId == 17 ? "bg-orange-500 text-white" : "bg-slate-200 hover:text-white"} hover:cursor-pointer p-4 hover:bg-orange-500`} whileHover={{ scale: 1.01 }}
                                whileTap={{ scale: 0.9 }}>
                                Prenotata
                            </motion.div>
                            <motion.div onClick={() => setDeal((prevState) => ({
                                ...prevState,
                                stageId: 18
                            }))} className={`rounded-2xl ${deal.stageId == 18 ? "bg-orange-500 text-white" : "bg-slate-200 hover:text-white"} hover:cursor-pointer p-4 hover:bg-orange-500`} whileHover={{ scale: 1.01 }}
                                whileTap={{ scale: 0.9 }}>
                                Uscita
                            </motion.div>
                            <motion.div onClick={() => setDeal((prevState) => ({
                                ...prevState,
                                stageId: 19
                            }))} className={`rounded-2xl ${deal.stageId == 19 ? "bg-orange-500 text-white" : "bg-slate-200 hover:text-white"} hover:cursor-pointer p-4 hover:bg-orange-500`} whileHover={{ scale: 1.01 }}
                                whileTap={{ scale: 0.9 }}>
                                Rientrata
                            </motion.div>
                        </div>

                        <p className="mt-4 mb-2 mb-1 font-light">Date noleggio</p>
                        ...
                        <motion.button onClick={() => {
                            const disponibile = checkRentDates()
                            if (disponibile) {
                                addDeal()
                            } else {
                                toast("Date non disponibili")
                            }
                        }} whileHover={{ scale: 1.01 }}
                            whileTap={{ scale: 0.99 }} className="rounded-xl bg-gradient-to-r from-emerald-300 from-10% via-emerald-400 via-30% to-emerald-500 to-100% p-1 text-black w-full p-2 text-white">Crea noleggio</motion.button>
                    </div>
                </div>
            </div >
        </>
