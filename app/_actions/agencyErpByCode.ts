...
export default async function agencyErpByCode(agencyCode: string) {
	try {
		...
		//TROVA AGENZIA E MOSTRA OUTPUT
		const result = data.data.agenzie.find((a: any) => {
			return a.code === agencyCode && a.isEnabled;
		});

		if (result) {
			return result.spokiConfiguration;
		} else {
			return null;
		}
	} catch (error: any) {
